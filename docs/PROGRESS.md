...
**Tests:** 360 .NET tests pass (289 unit, 71 integration, of which 50 run against a real MySQL database; none skipped),
plus headless browser runs of the dashboard (15 checks, M1) and the table designer (29 checks, M2).
...
- **The dev database contains test accounts and projects** (`smoke@…`, `member-smoke@…`, `ui-…@test.dev`, `ui-m2-…@test.dev` with "Bookshop" projects) from manual and UI checks.

## Next: M3 — Schema engine ⭐

Diff the draft (M2) against the last applied snapshot, render DDL from the typed definitions and
`ColumnDefault` values, show the plan, apply it with the `SchemaMigrations` journal, and detect drift.
